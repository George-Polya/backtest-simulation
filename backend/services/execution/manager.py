...
from backend.core.config import ExecutionProvider, Settings, get_settings
from backend.models.execution import ExecutionJob, ExecutionResult, JobStatus
from backend.providers.data.base import DataProvider, PriceData
from backend.services.execution.backend import ExecutionBackend, ExecutionError, LocalBackend
from backend.services.execution.docker_backend import DockerBackend, DEFAULT_PYTHON_IMAGE
from backend.services.execution.storage import InMemoryJobStorage, JobStorage, JobNotFoundError
    ...
    def create(
        ...
        if provider == ExecutionProvider.LOCAL:
            ...
            return DockerBackend(
                ...
                docker_socket_url=settings.docker_socket_url,
            )
    ...
    async def run_backtest(
        ...
        try:
            ...
            if final_tickers and start_date_str and end_date_str:
                ...
                logger.info(f"📊 Fetching data for {len(final_tickers)} tickers: {final_tickers}")
                ...
                if market_data:
                    ...
                    logger.info("✅ All ticker data validated and saved to workspace")
                else:
            ...
            try:
                ...
                if self._fallback_backend is not None:
                    logger.warning(
                        f"Docker execution failed: {primary_error}. "
                    ...
                    logger.info(f"Job {job.job_id} completed via local fallback execution.")
                else:
            ...
            # Update job with error, preserving error_code from ExecutionError
            ec = e.error_code if isinstance(e, ExecutionError) else None
            job.mark_failed(str(e), error_code=ec)
            await self._storage.update(job)
