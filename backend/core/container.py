...
class Container:
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        self._code_generation_backend: "CodeGenerationBackend | None" = None
    ...
    def get_code_validator(self) -> "ASTCodeValidator":
        ...
        if self._code_validator is None:
            from backend.services.code_validator import create_code_validator
    ...
    async def get_job_manager(self) -> "JobManager":
        ...
        if self._job_manager is None:
            from backend.services.execution.manager import create_job_manager
    ...
    async def get_code_generator(self) -> "BacktestCodeGenerator":
        ...
        Requires the data provider and code-generation backend to be initialized.
        ...
        if self._code_generator is None:
            ...
            self._code_generator = BacktestCodeGenerator(
                ...
                generation_backend=self.get_code_generation_backend(),
            )

        return self._code_generator

    def get_code_generation_backend(self) -> "CodeGenerationBackend":
        """
        Get or create the LangChain agent code-generation backend.

        Returns:
            CodeGenerationBackend instance (singleton per container)
        """
        if self._code_generation_backend is None:
            from backend.services.code_generation.langchain_agent_backend import (
                LangChainAgentCodeGenerationBackend,
            )

            self._code_generation_backend = LangChainAgentCodeGenerationBackend(
                api_key=self.settings.get_llm_api_key(),
                llm_config=self.settings.llm,
            )
        return self._code_generation_backend
    ...
    async def close_code_generator(self) -> None:
        ...
        self._code_generation_backend = None
