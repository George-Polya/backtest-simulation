...
class TestContainer:
    ...
    @pytest.mark.asyncio
    async def test_get_code_generator_injects_generation_backend(
        self,
        test_container: Container,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Test code generator is created with the agent backend seam."""
        ...
        monkeypatch.setattr(
            test_container,
            "get_code_generation_backend",
            lambda: fake_backend,
        )
        monkeypatch.setattr(
            test_container,
            "get_llm_provider",
            lambda: (_ for _ in ()).throw(AssertionError("legacy llm provider should not be fetched")),
        )

        generator = await test_container.get_code_generator()

        assert isinstance(generator, BacktestCodeGenerator)
        assert generator.generation_backend is fake_backend
