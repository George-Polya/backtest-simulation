...
from backend.services.execution.backend import ExecutionError
...
def _error_detail(message: str, error_code: ErrorCode) -> dict[str, str]:
    """Build a structured error detail dict for HTTPException."""
    return {"message": message, "error_code": error_code.value}
...
async def execute_backtest(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=_error_detail(str(e), ErrorCode.INVALID_REQUEST),
        )
    except NotImplementedError as e:
        logger.warning(f"Feature not implemented: {e}")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=_error_detail(str(e), ErrorCode.NOT_IMPLEMENTED),
        )
    ...
    try:
        ...
        try:
            error_code = ErrorCode(e.error_code) if e.error_code else ErrorCode.EXECUTION_FAILED
        except ValueError:
            error_code = ErrorCode.EXECUTION_FAILED
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(str(e), error_code),
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(f"Failed to execute backtest: {str(e)}", ErrorCode.INTERNAL_ERROR),
        )
...
async def get_job_status(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=_error_detail(f"Job not found: {job_id}", ErrorCode.JOB_NOT_FOUND),
        )
    ...
    try:
        ...
        if result.status != JobStatus.COMPLETED:
            raise HTTPException(
                ...
                detail=_error_detail(
                    f"Job is not completed yet. Current status: {result.status}",
        ...
        if not result.success or not result.data:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=_error_detail(
                    f"Job completed but no result data available: {result.error}",
                    ErrorCode.RESULT_FORMAT_FAILED,
                ),
            )
        ...
        if not equity_series_data:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=_error_detail(
                    "Missing equity_series in result data",
                    ErrorCode.RESULT_FORMAT_FAILED,
                ),
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(
                f"Failed to format results: {str(e)}",
                ErrorCode.RESULT_FORMAT_FAILED,
            ),
        )
    ...
    try:
        ...
        if result.status != JobStatus.COMPLETED or not result.data:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=_error_detail(
                    f"Job not completed or no data available. Status: {result.status}",
                    ErrorCode.JOB_NOT_COMPLETED,
                ),
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(
                f"Failed to get equity chart: {str(e)}",
                ErrorCode.RESULT_FORMAT_FAILED,
            ),
        )
...
async def get_drawdown_chart(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(
                f"Failed to get drawdown chart: {str(e)}",
                ErrorCode.RESULT_FORMAT_FAILED,
            ),
        )
...
async def get_monthly_returns(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_error_detail(
                f"Failed to get monthly returns: {str(e)}",
                ErrorCode.RESULT_FORMAT_FAILED,
            ),
        )
...
async def get_llm_providers() -> ConfigProvidersResponse:
    ...
    provider_descriptions = {
        "langchain": "LangChain - OpenRouter를 통한 다중 LLM 모델 접근 (Claude, GPT 등)",
    }
...
async def generate_backtest_code(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=_error_detail(
                f"Generated code failed validation: {'; '.join(e.errors)}",
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=_error_detail(f"Data not available: {str(e)}", ErrorCode.DATA_NOT_AVAILABLE),
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=_error_detail(str(e), ErrorCode.CODE_GENERATION_FAILED),
        )
