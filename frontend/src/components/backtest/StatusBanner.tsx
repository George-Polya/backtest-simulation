  ...
  if (isSubmitting) {
    return (
      <Card className="mb-4" title="Execution Status">
        <Alert title={isRetrying ? 'Retrying backtest' : 'Submitting backtest'} variant={isRetrying ? 'warning' : 'info'}>
          <div className="flex items-center gap-2">
            <LoadingSpinner size="sm" />
            <p>
              {isRetrying
                ? 'Docker environment is waking up. Automatically retrying the request...'
                : 'Sending backtest request to execution API.'}
            </p>
          </div>
        </Alert>
      </Card>
