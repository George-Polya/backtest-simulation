...
class TestGenerate:
    ...
    async def test_generate_with_custom_config(self, llm_config: LLMConfig) -> None:
        ...
        mock_custom_client.ainvoke = AsyncMock(return_value=mock_response)

        with patch(
            "backend.providers.llm.langchain_adapter.ChatOpenAI",
            side_effect=[mock_default_client, mock_custom_client],
        ):
            adapter = LangChainAdapter(
                api_key="test-key",
                llm_config=llm_config,
        ...
        # Verify a request-scoped client was created for the override.
        mock_custom_client.ainvoke.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_generate_prefers_reasoning_effort_over_max_tokens(
        self,
    ) -> None:
        ...
        extra_body = mock_client.ainvoke.call_args.kwargs["extra_body"]
