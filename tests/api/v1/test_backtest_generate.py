...
class TestGenerateBacktestCodeEndpoint:
    ...
    def test_generate_backtest_code_data_availability_error(
        ...
        with patch(
            ...
            assert response.status_code == 400
            assert "data not available" in response.json()["detail"]["message"].lower()
...
class TestGenerateEndpointIntegration:
    ...
    def test_generate_with_default_llm_settings(
        self, client, mock_code_generator, valid_backtest_request
    ):
        """Test generation uses backend-configured LLM provider."""
        with patch(
            "backend.core.container.Container.get_code_generator",
            return_value=mock_code_generator,
        ):
